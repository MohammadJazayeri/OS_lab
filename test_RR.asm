...
{ 
  ...
  20:	89 f3                	mov    %esi,%ebx
    {
        int pid = fork();
  22:	e8 f4 02 00 00       	call   31b <fork>

        if (pid < 0) 
  27:	85 c0                	test   %eax,%eax
  29:	78 3b                	js     66 <main+0x66>
        else if (pid == 0) 
        {
  ...
  30:	0f af db             	imul   %ebx,%ebx
  33:	83 ec 04             	sub    $0x4,%esp
  36:	8d 14 00             	lea    (%eax,%eax,1),%edx
  39:	53                   	push   %ebx
  3a:	52                   	push   %edx
  3b:	50                   	push   %eax
  3c:	e8 aa 03 00 00       	call   3eb <SJF_init>
    for (int r = 0; r < NUM_CHILDREN; r++) 
  41:	83 c4 10             	add    $0x10,%esp
  44:	83 fe 0a             	cmp    $0xa,%esi
  ...
  57:	78 20                	js     79 <main+0x79>
    for (int i = 0; i < NUM_CHILDREN; i++) {
  59:	83 c3 01             	add    $0x1,%ebx
  5c:	83 fb 0a             	cmp    $0xa,%ebx
  5f:	75 ef                	jne    50 <main+0x50>
            exit(); 
  61:	e8 bd 02 00 00       	call   323 <exit>
  ...
  a2:	bb 0a 00 00 00       	mov    $0xa,%ebx
  a7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  ae:	66 90                	xchg   %ax,%ax
                    sum += (i * j) % (r + 1);  // Nested operation
  b0:	89 c8                	mov    %ecx,%eax
  b2:	8b 75 e4             	mov    -0x1c(%ebp),%esi
                for (int j = 0; j < ITERATIONS / 10; j++) {
  b5:	01 f9                	add    %edi,%ecx
                    sum += (i * j) % (r + 1);  // Nested operation
  b7:	99                   	cltd   
  b8:	f7 7d d4             	idivl  -0x2c(%ebp)
  bb:	01 f2                	add    %esi,%edx
  bd:	89 55 e4             	mov    %edx,-0x1c(%ebp)
                for (int j = 0; j < ITERATIONS / 10; j++) {
  c0:	83 eb 01             	sub    $0x1,%ebx
  c3:	75 eb                	jne    b0 <main+0xb0>
            for (int i = 0; i < ITERATIONS; i++) {
  c5:	83 c7 01             	add    $0x1,%edi
  c8:	83 ff 64             	cmp    $0x64,%edi
  cb:	75 d3                	jne    a0 <main+0xa0>
  cd:	eb 92                	jmp    61 <main+0x61>
  cf:	90                   	nop
 ...
 4b9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 637:	90                   	nop
          s = "(null)";
 638:	ba 0c 08 00 00       	mov    $0x80c,%edx
        while(*s != 0){
